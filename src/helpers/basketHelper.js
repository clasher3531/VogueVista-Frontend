...
} from "../services/basketFetchService";
...
export async function addProductToBasket(product) {
  var basketUUID = getBasketUUID();
  var basketResponse = await addProduct(
    product.id,
    basketUUID ? basketUUID : null
  );
  if (basketResponse.error) {
    return null;
  }
...
export function validateShippingAddress() {
  ...
  if (
    currentBasket === null ||
    currentBasket.shippingAddress === null ||
    currentBasket.email === null
  ) {
    return false;
  } else {
